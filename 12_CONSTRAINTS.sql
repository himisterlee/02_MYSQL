...
-- NOT NULL
-- NULL 값을 허용하지 않는 조건
drop table if exists user_notnull; -- table 삭제하기
...
(
    user_no int not null unique, -- 기본키, unique 를 붙여주면 적용됨
    user_id varchar(255) not null,
    user_pwd varchar(255) not null,
    user_name varchar(255) not null,
    gender varchar(3),
...
-- 한 개의 컬럼에 설정할 수도 있고, 여러개의 컬럼을 묶어서 설정 할 수도 있다.(복합키)
...
(
    user_no int, -- 기본키
    -- user_no int primary key,    라고 작성도 가능
    user_id varchar(255) not null,
    user_pwd varchar(255) not null,
    user_name varchar(255) not null,
    gender varchar(3),
    phone varchar(255) not null,
    email varchar(255),
    primary key (user_no) -- 여기에 primary key 입력해줌
);
...
-- CONSTRAINT `user_foreignkey1_ibfk_1` FOREIGN KEY (`grade_code`) REFERENCES `user_grade` (`grade_code`)) 발생
insert into user_foreignkey1
(user_no, user_id, user_pwd, user_name, gender, phone, email, grade_code)
...
-- [23000][1452] Cannot add or update a child row: a foreign key constraint fails (`user_db`.`user_foreignkey1`,
-- CONSTRAINT `user_foreignkey1_ibfk_1` FOREIGN KEY (`grade_code`) REFERENCES `user_grade` (`grade_code`)) 발생
